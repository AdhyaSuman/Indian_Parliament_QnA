...
parser.add_argument('--min_df', type=float, default=0.001, help='to get the right data..minimum document frequency')
...
def get_topic_quality():
    ...
    with torch.no_grad():
        ...
        # cnt_all = []
        for tt in range(args.num_times):
            tc = get_topic_coherence(beta[:, tt, :].cpu().numpy(), train_tokens, vocab)
            TC_all.append(tc)
            # cnt_all.append(cnt)
        # print('TC_all.shape={},  TC_all: {}'.format(TC_all.shape, TC_all))
        print('\n')
        TC = np.mean(TC_all)
